...
def test_connect(signal_manager, mock_sender, mock_receiver):
    ...
    # Connect the signal
    result = signal_manager.connect(
        mock_sender, "test_signal", 
        mock_receiver, "on_signal"
    )
...
def test_disconnect_specific(signal_manager, mock_sender, mock_receiver):
    ...
    count = signal_manager.disconnect(
        mock_sender, "test_signal", 
        mock_receiver, "on_signal"
    )
...
def test_disconnect_by_receiver(signal_manager, mock_sender, mock_receiver):
    ...
    signal_manager.connect(
        mock_sender, "typed_signal", 
        mock_receiver, "on_typed_signal"
    )
...
def test_get_connections(signal_manager, mock_sender, mock_receiver):
    ...
    assert filtered_connections[0] == (
        mock_sender, "test_signal", mock_receiver, "on_signal"
    )

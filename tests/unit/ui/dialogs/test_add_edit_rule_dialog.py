...
from PySide6.QtWidgets import QDialog, QLineEdit, QComboBox, QRadioButton, QSpinBox, QMessageBox
...
class TestAddEditRuleDialog:
    ...
    def test_add_to_validation_list_button(self, qtbot, new_rule_dialog, mock_validation_service):
    ...
    def test_category_change_updates_validation_button(self, qtbot, new_rule_dialog):
        ...
        # Button should still be enabled for general with a to_value
        assert new_rule_dialog._add_to_validation_button.isEnabled()
    ...
    def test_cancel_button(self, qtbot, new_rule_dialog):
        ...
        # Use the cancel button directly
        cancel_button = new_rule_dialog._cancel_button
        assert cancel_button is not None
        assert cancel_button.text() == "Cancel"

        # Check if the button has a clicked signal connected to reject
        # This is hard to test directly, so we'll just check that the button exists
    ...
    def test_status_radio_buttons(self, qtbot, new_rule_dialog):
        ...
        # Use the set_status method instead of clicking
        new_rule_dialog.set_status("disabled")
        ...
        # Check changing back to enabled
        new_rule_dialog.set_status("enabled")

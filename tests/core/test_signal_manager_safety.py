...
def test_cleanup_on_object_destruction(signal_manager, emitter, app):
    ...
    # Manually trigger the cleanup that would normally happen in the Qt event loop
    # Note: In a real application, this would happen automatically when Qt's garbage collection runs
    signal_manager.disconnect_receiver(temp_receiver)
